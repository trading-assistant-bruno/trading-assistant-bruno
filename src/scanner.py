...
class Candidate:
    ...
    risk_eur: float
    eurusd: float
    entry_trigger_eur: float
    stop_limit_price: float
    stop_limit_price_eur: float
    buy_zone_max_eur: float
    stop_eur: float
...
def analyze_symbol(
    ...
    try:
        ...
        # Prix limite distinct de la zone stratégique d'achat.
        # On évite ainsi qu'un ordre stop-limit soit exécuté trop loin
        # au-dessus du déclenchement.
        stop_limit_slippage_pct = float(
            cfg(
                "strategy",
                "stop_limit_max_slippage_pct",
                0.50,
            )
        )

        stop_limit_price = entry * (
            1 + stop_limit_slippage_pct / 100
        )

        entry_trigger_eur = entry / eurusd
        stop_limit_price_eur = stop_limit_price / eurusd
        buy_zone_max_eur = buy_zone_max / eurusd
        stop_eur = stop / eurusd
        position_value_eur = position_value_usd / eurusd
        actual_risk_pct = (
            actual_risk_eur / capital_eur * 100
            if capital_eur > 0
            else 0.0
        )

        # L'alerte se place sur le pivot, pas sur la limite de l'ordre.
        alert_price = pivot
        ...
        return Candidate(
            ...
            entry_trigger_eur=round(entry_trigger_eur, 2),
            stop_limit_price=round(stop_limit_price, 2),
            stop_limit_price_eur=round(stop_limit_price_eur, 2),
            buy_zone_max_eur=round(buy_zone_max_eur, 2),
            stop_eur=round(stop_eur, 2),
...
def decide_final_action(candidate: Candidate) -> tuple[str, str]:
    """
    Sortie volontairement simple :
    - ACHAT — PLACER L'ORDRE
    - SURVEILLER
    - NE PAS ACHETER

    Les sous-motifs restent dans final_reason.
    """
    validation_cfg = CONFIG.get("validation", {})
    ...
    if candidate.status == "TROP ÉTENDU":
        candidate.action_label = "AUCUN ORDRE"
        return (
            "NE PAS ACHETER",
    ...
    if candidate.corporate_event_status == "REVIEW":
        candidate.action_label = "VÉRIFIER L'ÉVÉNEMENT"
        return (
            "SURVEILLER",
            candidate.corporate_event_reason
    ...
    if candidate.base_quality_score < min_base_score:
        ...
        return (
            "NE PAS ACHETER",
            (
                f"Structure insuffisante : qualité de base "
                f"{candidate.base_quality_score}/100 "
    ...
    if candidate.earnings_status == "INCONNU":
        candidate.action_label = "VÉRIFIER LES RÉSULTATS"
        return (
            "SURVEILLER",
            (
    ...
    if candidate.earnings_status == "PROCHE":
        candidate.action_label = "ATTENDRE APRÈS RÉSULTATS"
        return (
            "NE PAS ACHETER",
            (
    ...
    if candidate.status in {"DANS ZONE D'ACHAT", "PRÊT À DÉCLENCHER"}:
        candidate.action_label = "PLACER BUY STOP-LIMIT"
        return (
            "ACHAT — PLACER L'ORDRE",
            (
                "Setup validé. Utiliser l'ordre stop-limit calculé, "
                "sans dépasser le prix limite."
            ),
        )

    if candidate.status == "ATTENDRE":
        candidate.action_label = f"ALERTE À {candidate.alert_price:.2f} $"
        return (
            "SURVEILLER",
            (
                "Setup de qualité mais cassure non déclenchée. "
                "Placer une alerte au pivot et ne saisir aucun ordre."
            ),
        )

    candidate.action_label = "AUCUN ORDRE"
    return "NE PAS ACHETER", "Aucune condition d'achat valide."
...
<div class="card actionable">
  ...
  <div class="key-order">
    <div class="metric"><small>Type d'ordre</small><b>{{ c.order_type }}</b></div>
    <div class="metric"><small>Quantité</small><b>{{ c.shares }} actions</b></div>
    <div class="metric"><small>Déclenchement</small><b>{{ c.entry_trigger }} $ · {{ c.entry_trigger_eur }} €</b></div>
    <div class="metric"><small>Limite ordre</small><b>{{ c.stop_limit_price }} $ · {{ c.stop_limit_price_eur }} €</b></div>
    <div class="metric"><small>Stop-loss initial</small><b>{{ c.stop }} $ · {{ c.stop_eur }} €</b></div>
    <div class="metric"><small>Zone stratégique max</small><b>{{ c.buy_zone_max }} $ · {{ c.buy_zone_max_eur }} €</b></div>
    <div class="metric"><small>Capital engagé</small><b>{{ c.position_value_eur }} €</b></div>
    <div class="metric"><small>Risque réel</small><b>{{ c.risk_eur }} € · {{ c.actual_risk_pct }}%</b></div>
  </div>

  <p><b>ACTION :</b> {{ c.action_label }}</p>
...
<div class="card waiting">
  ...
  <div class="grid">
    ...
    <div class="metric"><small>Alerte à placer</small><b>{{ c.alert_price }} $</b></div>
    <div class="metric"><small>Déclenchement futur</small><b>{{ c.entry_trigger }} $</b></div>
    <div class="metric"><small>Ordre maintenant</small><b>AUCUN</b></div>
...
def send_telegram(
    ...
    actionable = [
        ...
        if c.final_decision == "ACHAT — PLACER L'ORDRE"
    ][:regime["new_positions_allowed"]]

    watch = [
        c for c in candidates
        if c.final_decision == "SURVEILLER"
    ]
    ...
    if regime["color"] == "ROUGE":
        ...
        for c in actionable:
            lines += [
                f"{c.ticker} — 🟢 ACHAT",
                f"{c.order_type} | {c.shares} actions",
                f"Stop {c.entry_trigger}$ | Limit {c.stop_limit_price}$",
                f"Stop-loss {c.stop}$ | Position ≈ {c.position_value_eur}€",
                f"Risque {c.risk_eur}€ ({c.actual_risk_pct}%)",
                "",
            ]
...
def main() -> None:
    ...
    # Respect du nombre maximal de nouvelles positions autorisées
    # par le régime de marché.
    ...
    rejected_candidates = [
        asdict(c)
        for c in candidates
        if c.final_decision == "NE PAS ACHETER"
    ]
